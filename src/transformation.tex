...
\end{example}
\begin{solution}
    We can try expressing each transformation in terms of $F_X(x)$, the distribution function of $X$.
    ...
    \begin{enumerate}
        ...
        \item The cumulative distribution function of $Y = X_+ = \max\{X, 0\}$ is
        \begin{align*}
            \mathbb{P}(X_+ \leq y) &= \mathbb{P}(\max\{X, 0\} \leq y)\\
            &= \begin{cases}
                \mathbb{P}(0 \leq y) = 0 &\text{if } y < 0,\\
                \mathbb{P}(X_+ = 0) = \mathbb{P}(X \leq 0) = F_X(0) &\text{if } y = 0,\\
                \mathbb{P}(0 < X \leq y) = F_X(y) &\text{if } y > 0.
            \end{cases}
        \end{align*}

        \item The cumulative distribution function of $Y = X_- = \min\{X, 0\}$ is
        \begin{align*}
            \mathbb{P}(X_- \leq y) &= \mathbb{P}(\min\{X, 0\} \leq y)\\
            &= \begin{cases}
                \mathbb{P}(X_- \leq 0) = 1 &\text{if } y > 0,\\
                \mathbb{P}(X \leq y) = F_X(y) &\text{if } y \leq 0.
            \end{cases}
        \end{align*}
    \end{enumerate}
\end{solution}
...
\end{example}
\begin{solution}
    The transformation $Y = X^2 + 3$ maps the set points $\mathcal{A} = \{0, 1, 2, \ldots \}$ 
    onto $\mathcal{B} = \{3, 4, 7, 12, \ldots \}$. The inverse mapping is 
    $X = \sqrt{Y - 3}$. Since there is no negative values in $\mathcal{A}$, so we take only the positive square root.

    Thus, the probability mass function of $Y$ is
    \begin{align*}
        \mathbb{P}(Y=y) &= \mathbb{P}(X = \sqrt{y - 3})\\
        &= \frac{e^{-\lambda}\lambda^{\sqrt{y - 3}}}{(\sqrt{y - 3})!}, \quad y \in \mathcal{B}.
    \end{align*}
\end{solution}

\begin{example}
    Given $X \sim BIN(n,p)$ for $x = 0,1,2,\ldots,n$. Find the probability mass function of
    \begin{enumerate}
        \item $Y = aX + b$,
        \item $Z = X^2$,
        \item $W = \sqrt{X}$.
...
\begin{solution}
    \begin{enumerate}
        \item Since $Y = aX + b$ maps the set points $\mathcal{A} = \{0, 1, 2, \ldots, n\}$ onto 
        $\mathcal{B} = \{b, a+b, 2a+b, \ldots, na+b\}$, the inverse mapping is $X = \frac{Y-b}{a}$. 
        And since there is no negative values in $\mathcal{A}$, so we take only the positive value. The 
        pdf of $Y$ is
        \[
            \mathbb{P}(Y=y) = \mathbb{P}\left( X = \frac{y-b}{a} \right) = {n \choose \mfrac{y-b}{a}} p^{\mfrac{y-b}{a}} (1-p)^{n - \mfrac{y-b}{a}}, \quad y \in \mathcal{B}.
        \]

        \item The transformation $Z = X^2$ maps the set points $\mathcal{A} = \{0, 1, 2, \ldots, n\}$ onto 
        $\mathcal{B} = \{0, 1, 4, 9, \ldots, n^2\}$. The inverse mapping is $X = \sqrt{Z}$. 
        Also since there is no negative values in $\mathcal{A}$, so we take only the positive square root. The 
        pdf of $Z$ is
        \begin{align*}
            \mathbb{P}(Z=z) &= \mathbb{P}(X^2 = z) \\
            &= \mathbb{P}(X = \sqrt{z})\\ 
            &= {n \choose \sqrt{z}} p^{\sqrt{z}} (1-p)^{n - \sqrt{z}}, \quad z \in \mathcal{B}.
        \end{align*}

        \item $W = \sqrt{X}$ maps the set points $\mathcal{A} = \{0, 1, 2, \ldots, n\}$ onto 
        $\mathcal{B} = \{0, 1, \sqrt{2}, \ldots, \sqrt{n}\}$. The inverse mapping is $X = W^2$. 
        Also since there is no negative values in $\mathcal{A}$, so we take only the positive square root. The 
        pdf of $W$ is
        \[
            \mathbb{P}(W=w) = \mathbb{P}(X = w^2)
            = {n \choose w^2} p^{w^2} (1-p)^{n - w^2}, \quad w \in \mathcal{B}.
        \]
    \end{enumerate}
\end{solution}
...
\begin{remark}
    \begin{enumerate}
        \item If the conditions of this theorem are violated, then we should return to the previous method.
        \item If the pdf $f$ vanishes outside an interval $[a,b]$ of finite length, we need only to 
        assume that $g=f(x)$ is differentiable in $(a,b)$ and either $g'(x) > 0$ or $g'(x) < 0$ throughout 
        the interval. Then we can take $\alpha = \min \{g(a), g(b)\}$ and $\beta = \max \{g(a), g(b)\}$.
    \end{enumerate}
\end{remark}
