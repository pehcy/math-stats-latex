...
sets is that using the \index{pivotal quantities}.
...
\begin{mdframed}
 ...
 Construct a 95\% confidence interval for the mean calcium content in the water.
    ...
    \begin{Exercise}
         The executives at Aperture Inc. having recently solved their widget crises, have another 
major problem with one of their products. Many cities are sending complaints that their manhole 
covers are defective and people are falling into the sewers. Aperture Inc. is pretty sure that 
only 4\% of their manhole covers are defective, but they would like to do a study to confirm this 
number. They are hoping to construct a 95\% confidence interval to get within 0.01 of the true 
proportion of defective manhole covers.  How many manhole covers need to be tested?
    \end{Exercise}
    ...
    \begin{Exercise}
        Let $X_1, X_2, \ldots, X_n$ be a random sample from population 
        with Pareto density function
        \[
                f_{X_i}(x|\theta, \beta) = \begin{cases}
                    \theta \, \beta^\theta \, x^{-(\theta + 1)} & x \geq \beta\\
                    0 & \text{otherwise,}
                \end{cases}
        \]
        Where $\theta$ is an unknown parameter and $\beta$ is a known constant. Show that
        \[
            \left[ \frac{2 \sum^n_{i=1} \ln \left(\frac{X_i}{\beta}\right)}{\chi^2_{1 - \mfrac{\alpha}{2} (2n\beta)}},\> \frac{2 \sum^n_{i=1} \ln \left(\frac{X_i}{\beta}\right)}{\chi^2_{\mfrac{\alpha}{2} (2n\beta)}} \right]
        \]
        is a $100(1-\alpha)\%$ confidence interval for $\mfrac{1}{\theta}$.
    \end{Exercise}

    \begin{Exercise}
        Let $X_1, X_2, \ldots, X_4$ be a random sample of sample size $n=4$ from population 
        ...
        Construct a $90\%$ confidence interval for $\theta$ if the observed sample data is
        \[
