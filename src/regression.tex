...
statistic, thus they are \index{UMVUE} of $\alpha$ and $\beta$, respectively.
...
In addition to estimating the slope parameters $\beta_0$ and $\beta_1$, an estimate of $\sigma^2$ is required to test hypotheses and 
construct confidence interval estimates pertinent to the regression model. For example, the population regression model is 
\begin{equation}
    y_i = \beta_0 + \beta_1 x_i + \varepsilon_i \quad \text{where } \varepsilon_i \sim NID(0, \sigma^2).
\end{equation}
The variance $\sigma^2$ measure the variability of the random error $\varepsilon_i$ (thus, variability of $y_i$) and can be estimated based on the 
\textbf{residual} or \textbf{error sum of squares}. The error sum of squares (equivalently, \textit{residual sum of squares}), denoted by $\CS{E}$, is 
\begin{equation}
...
\begin{example}
    Table below shows the cost of installing a sample of communications nodes for a large manufacturing firm. The number 
    of access ports at each of the sampled nodes is also shown. The data scientist wants to develop a linear model 
    that is helpful in pricing the installation of new communications nodes on the network.
    \begin{center}
        \begin{tabular}{cc|cc}
    ...
    \begin{enumerate}
        \item What is the regression line that relate the number of ports to cost.
        \item Is there sufficient evidence to conclude that a linear relationship exists between the cost and number of ports 
            under $\alpha = 0.05$?
        \item Test whether there is a direct (positive) relationship between cost and number of ports. Use $\alpha =0.05$.
        \item A claim is made that each new access port adds at least $\$1,000$ to the installation cost of a communications node. 
            Examine this claim at $\alpha = 0.05$.
        \item The following test has no practical significance in this problem and is performed merely to illustrate the test for the intercept provided on 
            computer output. Test whether the intercept is zero.
    \end{enumerate}
\end{example}

\begin{solution}
    \begin{enumerate}
        From the outputs provided above:

        \item The regression line is $\widehat{y}_i = 16596.84 + 644.64x_i$.
        \item The test hypotheses are $H_0: \beta_1 = 0$ versus $H_1 : \beta_1 \neq 0$.
        ...
        We reject $H_0$, we have sufficient evidence to conclude that a linear relationship exists between the cost and 
        the number of ports.

        \item We want to test $H_0 : \beta_1 \leq 0$ against $H_1 : \beta_1 > 0$.
        \[
            p-\text{value} = \frac{8.88 \times 10^{-6}}{2} = 4.44 \times 10^{-6} < 0.05.
        \]
        Reject $H_0$, we have sufficient evidence to conclude that there is a direct (positive) relationship 
        between the cost and the number of ports.

        \item The test hypotheses are $H_0: \beta_1 \geq 1000$ versus $H_1 : \beta_1 < 1000$.
        \[
            T = \frac{\widehat{\beta}_1 - \beta^*_1}{\sqrt{\displaystyle \frac{\MS{E}}{S_{xx}}}}
        \]
    \end{enumerate}
\end{solution}
...
\section{Regression Analysis --- Analysis of Variance Approach}

Beside $t$-test, we may also use an \textbf{ANOVA} to test significance of regression. The analysis of variance is based on a partitioning 
of total variability in the response variable $y$. 

\subsection{Decomposition of Sum of Squares}
One useful aspect of SLR is that it can decomposite the variation into two parts: 
the variation of the predicted values, and the variation of prediction errors.

If all such deviations are squared, the squared deviations $(y_i - \overline{y})^2$ will provide the 
basis for measuring the spread of the data.

Squaring again both sides of the preceeding equation and sum over all $i = 1, 2, ..., n$, we can obtain the 
following
...
After squaring the right-hand side of the equation, we need some algebraic calculation to evaluate the cross-product 
term $(y_i - \hat{y}_i)(\hat{y}_i - \overline{y})$.

\begin{align*}
    \sum(y_i - \hat{y}_i)(\hat{y}_i - \overline{y}) &= \sum(y_i - \hat{\beta}_0 - \hat{\beta}_1 x_i)\, (\hat{\beta}_0 + \hat{\beta}_1 x_i - \overline{y})\\
    &= \sum [y_i - \overline{y} - \hat{\beta}_1 (x_i - \overline{x})]\, [\hat{\beta}_1 (x_i - \overline{x})]\\[0.7em]
    &= \hat{\beta}_1 \, \text{SS}_{xy} - \hat{\beta}^2_1\, \text{SS}_{x}\\
    &= 0
\end{align*}
this implies that the sum of the cross-products over all response variables is zero 
\footnote{In linear algebra, this implies the residuals are orthogonal to the regressor.}. And hence, the finalized equation will be 
...
\begin{center}
    \begin{itemize*}
        \item[] TSS
            \tikz\node [fill=gray!20,draw, circle] (n1) {};
        \item[\quad] RSS
            \tikz\node [fill=green!20,draw, circle] (n2) {};
        \item[\quad] RegSS
            \tikz\node [fill=cyan!20,draw, circle] (n3) {};
    \end{itemize*}
\end{center}
...
\begin{itemize}
    \item \textbf{Total Sum of Squares (TSS)} represents the variation of $Y$, that is, the sum of squares in all of the responses. It measures
    the amount of variability inherent in the response prior to performing regression.
    \item \textbf{Residual Sum of Squares (RSS)}, also called the squared error loss or the L2-norm, is the variation of all 
    the response values about the fitted regression line. It describes how well the model fits the data.
    \item \textbf{Regression Sum Of Squares (RegSS)}, the difference between TSS and RSS, or the total variation 
    explained through knowledge of $x$. RegSS measure how effective the SLR model is in explaining the variation of $y$.
\end{itemize}

Since the TSS is fixed, and both RSS and RegSS always non-negative as they are sum of squares. The higher the RegSS of 
a regression model, the lower its RSS. In other word, when RSS is small, the prediction will be closer to actual value, and the model 
is a good fit. Else if RSS is large, the prediction is not accurate and this model is a poor fit.

You might be wondering why it should be the squared error? Why not the absolute error or the cubed error? In fact, 
they both can be used. But only the squared one maximized likelihood for betas.

\subsection{Mean Square Error}
An unbiased estimator of the unknown error variance $\sigma^2$, the mean square error (MSE) is derived as
\begin{equation}
    s^2 = \text{MSE} = \frac{\text{RSS}}{n-2} = \frac{1}{n-2} \sum^{n}_{i=1} e^2_i \> = \frac{1}{n-2} \sum^{n}_{i=1} (y_i - \hat{y}_i)^2
\end{equation}
Note that in defining 
MSE we have divided $n-2$ instead of $n-1$. This is because we need at least two observations to determine the 
fitting line.
